...
Main.QryEval is the main class. Given a parameter file which specifies the
...
QryOperator.QryopIl and QryOperator.QryopSl are extensions of QryOperator.Qryop that are specialized for
query opeators that return inverted lists (e.g., TERM, SYN, NEAR/n)
and query operators that return score lists (e.g., AND, SCORE).

QryopILTerm, QryOperator.QryopIlSyn, and QryOperator.QryopSlAnd are query operator
...
DataStructure.QryResult objects that encapsulate both types of result.  Some query
...
how the behavior of QryOperator.QryopSlScore and QryOperator.QryopSlAnd can be altered
...
example, to implement the Indri retrieval model, do the following.
  ...
  * Modify the QryOperator.QryopSlScore function to calculate a query likelihood
    score with Dirichlet smoothing, and to calculate default scores.

  * Modify the evaluate method of each query operator of type QryOperator.QryopSl
...
The DataStructure.ScoreList class provides a very simple implementation of a score
list.

The DataStructure.InvList class provides a very simple implementation of an inverted
list.
...
DataStructure.TermVector class that provides a simple, Indri-like API that gives
