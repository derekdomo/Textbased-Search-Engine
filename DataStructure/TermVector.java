...
/**
 *  The DataStructure.TermVector class provides an Indri DocVector-style interface
...
public class TermVector {
  ...
  /**
   *  Constructor.  Create a DataStructure.TermVector for a field in a document.
   *  @return {@link TermVector}
   */
  public TermVector(int docId, String fieldName) throws IOException {

    //  Fetch the term vector.
    this.luceneTerms = MainEval.READER.getTermVector(docId, fieldName);
  ...
  public long totalStemFreq(int i) throws IOException {
    long freq=MainEval.READER.totalTermFreq(terms[i]);
    return freq;
  }
