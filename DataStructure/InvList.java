...
public class InvList {
  ...
  public InvList(String termString, String fieldString) throws IOException {
    ...
    if (MainEval.READER.docFreq(term) < 1)
      return;
    //  Lookup the inverted list.
    DocsAndPositionsEnum iList =
      MultiFields.getTermPositionsEnum(MainEval.READER,
				       MultiFields.getLiveDocs(MainEval.READER),
    ...
    //iterate all the doc to transfer all the info to invertedList
  ...
  public int getDocid(int n) {
    ...
    if (this.postings.size()==n)
        return Integer.MAX_VALUE;
    return this.postings.get(n).docid;
  }

 /**
  * Get the n'th document posting from the inverted list.
  * @param n The index of the requested document.
  * @return the internal document posting.
  */
  public DocPosting getDoc(int n) { return this.postings.get(n);}
