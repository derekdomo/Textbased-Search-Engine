...
/**
 * Main.DocLengthStore is used to access the document lengths of indexed docs.
 */
public class DocLengthStore {
    ...
    /**
     * @param reader IndexReader object created in {@link MainEval}.
     */
    ...
    /**
     * Returns the total length of the whole document collections
     *
     * @param fieldname Namr of field to access lengths. "body" is the default
     * @return totallength
     */
    public long getDocLength(String fieldname) {
        long res = 0;
        int max = MainEval.READER.maxDoc();
        for (int i = 0; i < max; i++) {
            res += values.get(fieldname).get(i);
        }
        return res;
    }
}
